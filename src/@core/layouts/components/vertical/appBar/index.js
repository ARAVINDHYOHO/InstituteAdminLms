...
const LayoutAppBar = props => {
  ...
  const appBarBlurEffect = appBarBlur && {
    '&:after': {
      ...
      mask: theme =>
        `linear-gradient(${theme.palette.background.layout}, ${theme.palette.background.layout} 18%, transparent 100%)`,
      background: theme =>
        `linear-gradient(180deg,${hexToRGBA(theme.palette.background.layout, 0.7)} 44%, ${hexToRGBA(
          theme.palette.background.layout,
          0.43
        )} 73%, ${hexToRGBA(theme.palette.background.layout, 0)})`
    }
  }
  if (appBar === 'hidden') {
    return null
